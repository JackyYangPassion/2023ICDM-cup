...
def main(args):
    ...
    for epoch in range(args.n_ggd_epochs):
        ...
        if loss < best:
            ...
            best_t = epoch
            cnt_wait = 0
            torch.save(ggd.state_dict(), 'best_ggd' + tag + '.pkl')
        else:
            cnt_wait += 1
    ...
    ggd.load_state_dict(torch.load('best_ggd' + tag + '.pkl'))
    ...
    torch.save(embeds,args.data_root_dir + '/'+'x.pt')
    torch.save(labels,args.data_root_dir + '/'+'y.pt')
    dur = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root_dir', type=str, default='./pretrained_embeddings/umap_embedded_datasets/ogb',

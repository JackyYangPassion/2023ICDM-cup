...
def load_icdm_data():
    """
        ...
        returns:
            ...
            d: dimension number of node attributes
        """
    nodes_data = pd.read_csv('data/icdm2023_session1_test/icdm2023_session1_test_node_feat.txt', header=None)
    edges_data = pd.read_csv('data/icdm2023_session1_test/icdm2023_session1_test_edge.txt', header=None)

    g = dgl.graph((edges_data[0].to_numpy(), edges_data[1].to_numpy()))
    g = dgl.add_self_loop(g)
    g.ndata['feat'] = torch.FloatTensor(nodes_data.values)
